...
const Page: NextPageWithLayout = () => {
  ...
  const handleCallApi = async () => {
    try {
      setOnSending(true)
      const data: any = await instance.get('/services')
      setMeta(data.meta)
      setJobFromApi(data.data)
      setOnSending(false)
    } catch (error) {
      console.log(error)
  ...
  const handlePostApi = async (payload: any) => {
    try {
      setOnSending(true)
      const data: any = await instance.post('/industries', payload)
      setMeta(data.meta)
      setJobFromApi(data.data)
      setOnSending(false)
    } catch (error) {
  ...
  return (
    <>
      <div className="relative h-[calc(88vh)]">
        <div className="flex justify-between items-end">
          <div className="flex flex-col text-base-black-1">
            <p className=" text-base">Danh sách Nghề</p>
            <p className="text-lg font-semibold">1200 nghề</p>
          </div>
        ...
        <div className="table-card my-4">
          <div className="grid grid-cols-3 gap-4 ">
            {jobFromApi.length > 0 ? (
              jobFromApi.map((item: any, index: any) => (
                <div
