...
const TableComponent: React.FC<TableType> = ({ rowsPerPage, initialData, columns, labelTable, multiSelectTable, renderCell, onRowAction }) => {
  ...
  return (
    ...
    >
      <TableHeader columns={columns}>
        {(column) => 
          <TableColumn key={column.id} allowsSorting={column.sortable}>{column.name}</TableColumn>
        }
      </TableHeader>
      <TableBody items={items} emptyContent={"Chưa có dữ liệu"}>
        {(item) => (
          <TableRow key={item.id}>
            {(columnKey) => <TableCell>{renderCell ? renderCell(item, columnKey) : (item as any)[columnKey]}</TableCell>}
          </TableRow>
        )}
      </TableBody>
    </Table>
